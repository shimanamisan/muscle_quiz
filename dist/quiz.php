...
class PhysicalMonster extends Physical
{
  ...
  public function __construct($name)
  {
    ...
    if (!isset($_SESSION['current_num'])) {
      ...
      debug(
        'SESSIONをセットしました construct処理(36)：' . print_r($_SESSION, true)
      );
      debug('   ');
      debug('   ');
    }
  ...
  public function getCorrectAnswer()
  {
    switch (true) {
      case $_SESSION['easy'] === 1:
        return round(
          ($_SESSION['correct_count'] / count($this->questionEasy)) * 100
        );

      case $_SESSION['normal'] === 2:
        return round(
          ($_SESSION['correct_count'] / count($this->questionNormal)) * 100
        );

      case $_SESSION['hard'] === 3:
        return round(
          ($_SESSION['correct_count'] / count($this->questionHard)) * 100
        );
  ...
  public function checkAnswer()
  {
    switch (true) {
      case $_SESSION['easy'] === 1:
        ...
        debug(
          '$_SESSION[current_num]チェック前' .
            print_r($_SESSION['current_num'], true)
        );
        debug('   ');

        $_SESSION['current_num']++;
        debug(
          '$_SESSION[current_num]チェック後' .
            print_r($_SESSION['current_num'], true)
        );
        debug('   ');
      ...
      case $_SESSION['normal'] === 2:
        $correct_answer =
          $this->questionNormal[$_SESSION['current_num']]['a'][0];
        if ($correct_answer === $_POST['answer']) {
          $_SESSION['correct_count']++;
        }
        debug(
  ...
  private function setUPeasy()
  {
    ...
    $this->questionEasy[] = [
      'Q' =>
        'Ｑ：トレーニング後、適切な休息をとることで筋力の向上や筋肥大などの効果が現れる現象をなんと呼ぶか？',
      'a' => ['超回復', '即回復', '全回復', '急回復'],
    ];
    $this->questionEasy[] = [
      'Q' =>
        'Ｑ：トレーニング後、○○分以内にタンパク質を摂取することが筋タンパク質合成のゴールデンタイムとされているか？',
      'a' => ['60分', '30分', '15分', '5分'],
    ];
    $this->questionEasy[] = [
    ...
    $this->questionEasy[] = [
      'Q' =>
        'Ｑ：ビタミン、ミネラル、食物繊維を豊富に含む和名が「メハナヤサイ」と呼ばれる緑黄色野菜は？',
  ...
  private function setUPnormal()
  {
    $this->questionNormal[] = [
      'Q' =>
        'Ｑ：筋トレにはBIG3と呼ばれる種目があり、ベンチプレス、スクワット、あと一つは何か？',
      'a' => [
        'デッドリフト',
        'プルダウン',
        'シーテッドローイング',
    ...
    $this->questionNormal[] = [
      'Q' =>
        'Ｑ：ランニングのような有酸素運動は、一般的に何分以上行うことが脂肪燃焼に良いとされているか？',
      'a' => ['20分', '10分', '15分', '30分'],
    ];
    $this->questionNormal[] = [
      'Q' =>
        'Ｑ：トレーニング時間の短縮及び、心肺機能の向上も期待できるセット法を何というか？',
      'a' => [
        'スーパーセット法',
        'マルチセット法',
        'ファーストレップス法',
    ...
    $this->questionNormal[] = [
      'Q' =>
        'Ｑ：同一筋系統の種目を2つ続けて行うことを1セットとするセット法を何というか？',
      'a' => [
        'コンパウントセット法',
        'ジャイアントセット法',
        'フォーストレップス法',
    ...
    $this->questionNormal[] = [
      'Q' =>
        'Ｑ：トレーニング後、筋タンパク質の合成感度が上昇するが、少なくとも何時間継続することが明らかにされているか？',
  ...
  private function setUPhard()
  {
    $this->questionHard[] = [
      'Q' =>
        'Ｑ：筋肉の合成を上回る量で、筋肉の分解が行われている状態を何というか。',
      'a' => ['カタボリック', 'アナボリック', 'メタボリック', 'シンボリック'],
    ];
    $this->questionHard[] = [
      'Q' =>
        'Ｑ：総クレアチン量は最大○○グラムでまで増やせると推測されているか？',
    ...
    $this->questionHard[] = [
      'Q' =>
        'Ｑ：筋肉の分解を上回る量で、筋肉の合成が行われている状態を何というか。',
